...
export default function Hospitals() {
  ...
  const handleSubmit1 = (e) => {
    ...
    var templateParams = {
      ...
      place: appointDetails.centerName,
  };
   
  emailjs.send('service_ap98qjf', 'template_f0jl5k8', templateParams)
      .then(function(response) {
         console.log('SUCCESS!', response.status, response.text);
      }, function(error) {
         console.log('FAILED...', error);
      });
    //     emailjs.sendForm("service_ap98qjf", "template_f0jl5k8", form.current, "rFBkfkgj_F7sUakLv").then(
    //   (result) => {
        ...
        <div className="modal fade" id="exampleModalToggle" aria-hidden="true" aria-labelledby="exampleModalToggleLabel" tabIndex="-1" >
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content">
              ...
              <form onSubmit={handleSubmit1} ref={form}>
                <div className="modal-body">
                  <label className="form-label" htmlFor="name"> {t("Your Name")}: </label>
                  <input className="form-control w-100" type="name" name="name" value={appointDetails.name} onChange={handleChange} id="name" ></input>
                  <label className="form-label" htmlFor="date">
                    {t("Choose Date")}:
                  </label>
                  <input className="form-control w-100" type="date" name="date" value={appointDetails.date} onChange={handleChange} id="date" ></input>
                  <label className="form-label" htmlFor="time">
                    {t("Choose time")}:
                  </label>
                  <input className="form-control w-100" type="time" name="time" value={appointDetails.time} onChange={handleChange} id="time" step="1800" ></input>
                  <label htmlFor="exampleDataList" className="form-label" >
                    {t("Specialty")}:
                  </label>
                  <input name="specialty" value={appointDetails.specialty} onChange={handleChange} className="form-control w-100" list="datalistOptions" id="exampleDataList" placeholder={t("Type to search...")} autoComplete="off"/>
